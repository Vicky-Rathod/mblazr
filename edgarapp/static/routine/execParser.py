...
try:
    ...
    mycursor.execute("SELECT cik, filingtype, name, MAX(CAST(filingdate AS datetime)) FROM edgarapp_filing GROUP BY cik, filingtype, name HAVING filingtype = %s ORDER BY name", ['DEF 14A'])

    myresult = mycursor.fetchall()

    mydb.close()
    
    for cik, filingtype, company, filingdate in myresult:
        if (filingtype == 'DEF 14A' and company != 'BOSTON BEER CO INC'):
            mydb = mysql.connector.connect(host='172.104.7.112',
                                         database='edgarData',
                                         user='reguser',
                                         password='Edgar@2020')
            ...
            if str(today.year) == date.split('-')[0]:
                mycursor.execute("SELECT executives FROM edgarapp_executives WHERE company = %s", [company])
                print(len(mycursor.fetchall()), company, date)
                if mycursor.rowcount == 0:
                    new_html_text = r"https://www.sec.gov/Archives/edgar/data/" + parts[0] + '/' + full_accession + '/' + dateRemove
                    print('#######################STARTING NEW FILE###############################')
                    print(new_html_text)

                    # new_html_text = r"https://www.sec.gov/Archives/edgar/data/1569994/000156999420000012/0001569994-20-000012.txt"
                    # new_html_text = r'https://www.sec.gov/Archives/edgar/data/3453/000155837020002272/0001558370-20-002272.txt'
                    # grab the response
                    ...
                    try:
                        ...
                        for document_id in filing_documents:
                            ...
                            if document_id == 'DEF 14A':
                                for page_num in pages_dict:
                                    ...
                                    # print(str(page_code))
                                    cleaned_document = cleaned_document + str(page_code)
                        # print(master_filings_dict[accession_number]['filing_documents']['DEF 14A']['pages_code'])
                        directory = '/mnt/filings/files/' + str(cik) 
                        print(cik)
                        ...
                        dateFiled = str(filingdate).split(' ')[0]
                        localpath = directory+'/'+dateFiled+'-parsed_proxy.txt'  # directory/filingdate-filename
                        if not os.path.isfile(localpath):
                            open(localpath, 'w').write(cleaned_document)
                        ...
                        for document_id in filing_documents:
                            ...
                            if document_id == 'DEF 14A':
                                ...
                                for page_num in pages_dict:
                                    ...
                                    # print(directors_found)
                                    for element in directors_found:
                                        matches = ['Summary', 'Compensation', 'Table'] 
                                        capsMatch = ['SUMMARY', 'COMPENSATION', 'TABLE']
                                        lowerMatch = ['summary', 'compensation', 'table']
                                        notMatches = ['Table of Contents', 'TABLE OF CONTENTS']
                                        if done == False and (all(x in element.text for x in matches) or all(x in element.text for x in capsMatch) or all(x in element.text for x in lowerMatch)) and not all(x in element.text for x in notMatches): 
